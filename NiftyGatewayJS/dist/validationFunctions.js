...
function testAndCleanPurchaseForObject(TxObject) {
  ...
  if (typeof TxObject.purchaseForFunction == 'undefined' && typeof TxObject.transactionData == 'undefined') {
    ...
    var url = 'https://devportalbackend.niftygateway.com/utils/get-generic-abi'; //make contract from data
    ...
    var fun_sig = TxObject.purchaseForFunction; //check for function
    ...
    if (sigExists.sigExists == true) {
      ...
      var userAdd = Utils.toChecksumAddress('0x0000000000000000000000000000000000000000'); //prep args
...
function testAndCleanTransactionObject(TxObject) {
  ...
  if (typeof TxObject.regularTransactionFunction == 'undefined' && typeof TxObject.transactionData == 'undefined') {
    ...
    var fun_sig = TxObject.regularTransactionFunction; //check for function
...
function generateJSONInterfaceFromSig(fun_sig) {
  ...
  for (var j = 0; j < argLocs.length - 1; j++) {
    ...
    inputDict.type = fun_sig.substring(argLocs[j] + 1, argLocs[j + 1]); // var input = fun_sig.substring((argLocs[j]+1),argLocs[j+1])
...
function prepareArgsFakeCall(arg_list, abi_inputs, user_address) {
  ...
  var new_args_list = [];
  var errors_list = []; //check to make sure enough args were submitted
